...
export class SubmissionService {
    private config: SubmissionConfig = {
        ...
        endpoints: {
            ...
            stepComplete: '/kyc/workflow-submissions/steps/{sessionId}/{stepId}/complete'
        }
    }

    private submissionState = new BehaviorSubject<{ [stepRef: string]: SubmissionResult }>({})
    ...
    async submitArtifact(payload: ArtifactPayload): Promise<SubmissionResult> {
        try {
            console.log('🎯 SUBMISSION-SERVICE: Submitting artifact:', payload.stepRef, payload.artifactType)
            
            const endpoint = payload.artifactType === 'selfie' 
                ? this.config.endpoints.selfie 
                : this.config.endpoints.document

            const response = await this.http.post<any>(`${this.config.baseUrl}${endpoint}`, {
            ...
            console.log('🎯 SUBMISSION-SERVICE: Artifact submitted successfully:', result.artifactId)
            return result

        } catch (error) {
            console.error('🎯 SUBMISSION-SERVICE: Artifact submission failed:', error)
            const result: SubmissionResult = {
                success: false,
     ...
     */
    async submitValidation(payload: ValidationPayload): Promise<SubmissionResult> {
        try {
            console.log('🎯 SUBMISSION-SERVICE: Submitting validation:', payload.validationType, payload.stepRef)
            
            const endpoint = payload.validationType === 'face_match' 
            ...
            this.updateSubmissionState(payload.stepRef, result)
            console.log('🎯 SUBMISSION-SERVICE: Validation completed:', result.validationResult)
            return result
     ...
     */
    async submitAnswers(payload: QuestionnairePayload): Promise<SubmissionResult> {
        try {
            console.log('🎯 SUBMISSION-SERVICE: Submitting questionnaire answers:', payload.stepRef)
            
            const response = await this.http.post<any>(`${this.config.baseUrl}${this.config.endpoints.questionnaire}`, {
                stepRef: payload.stepRef,
            ...
            this.updateSubmissionState(payload.stepRef, result)
            console.log('🎯 SUBMISSION-SERVICE: Questionnaire submitted successfully')
            return result

        } catch (error) {
            console.error('🎯 SUBMISSION-SERVICE: Questionnaire submission failed:', error)
            const result: SubmissionResult = {
                success: false,
                error: error instanceof Error ? error.message : 'Unknown error'
            }
     ...
     */
    async completeStep(sessionId: string, stepId: string, data: any): Promise<SubmissionResult> {
        try {
            ...
            console.log('🎯 SUBMISSION-SERVICE: Step completed successfully:', stepId)
            return result

        } catch (error) {
            console.error('🎯 SUBMISSION-SERVICE: Step completion failed:', error)
            const result: SubmissionResult = {
                success: false,
                error: error instanceof Error ? error.message : 'Unknown error'
            }
    ...
    private updateSubmissionState(stepRef: string, result: SubmissionResult): void {
        const state = this.submissionState.value
